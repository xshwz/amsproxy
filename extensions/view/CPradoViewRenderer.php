...
class CPradoViewRenderer extends CViewRenderer
{
	...
	protected function generateViewFile($sourceFile,$viewFile)
	{
		...
		for($i=0;$i<$n;++$i)
		{
			...
			elseif(strpos($str,'<%?')===0)	// expression
				$this->_output.=$this->processIssetExpression($match[1][0],$match[1][1]);
	 ...
	 * @param string $offset offset
	 */
	private function processIssetExpression($expression,$offset)
	{
		return $this->generatePhpCode("if (isset({$expression})) echo {$expression};",$offset);
	}
